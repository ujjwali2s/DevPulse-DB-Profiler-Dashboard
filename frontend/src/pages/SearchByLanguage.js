...
const SearchByLanguage = () => {
  ...
  const handleSearch = () => {
    if (!searchTerm.trim()) {
      setMessage('Please enter a language to search.');
      setBooks([]);
      return;
    }
    ...
    fetch(`http://localhost:5000/search/search-books?language=${encodeURIComponent(searchTerm)}`)
      .then(res => {
  ...
  return (
    <div className="search-title-container">
      <h1 className="gothic-header">Search Books by Language</h1>
