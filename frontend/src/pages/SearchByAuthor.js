...
// //   const handleSearch = () => {
// //     if (!authorTerm.trim()) {
// //       setMessage('Please enter an author name to search.');
// //       setBooks([]);
// //       return;
// //     }
...
// //     fetch(`http://localhost:5000/search/search-books?author=${encodeURIComponent(authorTerm)}`)
// //       .then(res => {
...
// //       <h1 className="gothic-header">Search Books by Author</h1>
...
// //               <p><strong>Author(s):</strong> {book.author_names || 'N/A'}</p>
// //               <p><strong>Genres:</strong> {book.genre_names || 'N/A'}</p>
// //               <p><strong>Publication Year:</strong> {book.publish_year}</p>
...
// import React, { useState } from 'react';
// import './SearchByTitle.css'; // Reusing same CSS for consistency
...
//     try {
//       const res = await fetch(`http://localhost:5000/search/search-books?author=${encodeURIComponent(author.trim())}`);
...
//   const handleClickDetails = async (type, id) => {
//     if (!id) return;
//     try {
//       const res = await fetch(`http://localhost:5000/search/${type}-details/${id}`);
