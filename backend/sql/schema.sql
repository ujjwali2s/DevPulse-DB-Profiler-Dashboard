...
CREATE TABLE Staff (
  ...
  Account_id INT,
  Branch_id INT,
  FOREIGN KEY (Person_id) REFERENCES Person_Info(Person_id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE Member (
  ...
  Account_id INT,
  FOREIGN KEY (Person_id) REFERENCES Person_Info(Person_id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (Status_id) REFERENCES Membership_Status(Status_id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (Account_id) REFERENCES Account(Account_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE Borrow_record (
  ...
  FOREIGN KEY (Member_id) REFERENCES Member(Member_id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (Staff_id) REFERENCES Staff(Staff_id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (Copy_id) REFERENCES Book_Copy(Copy_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE Fine_Calculated (
  ...
  FOREIGN KEY (Borrow_id) REFERENCES Borrow_record(Borrow_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE return_lost_requests (
  ...
  borrow_id INT NOT NULL REFERENCES borrow_record(borrow_id),
  request_type VARCHAR(10) NOT NULL CHECK (request_type IN ('return', 'lost')),
  status VARCHAR(10) NOT NULL DEFAULT 'pending',  -- pending, approved, rejected
  request_date DATE NOT NULL DEFAULT CURRENT_DATE,
  staff_id INT,  -- staff who processed it
  processed_date DATE
);

ALTER TABLE Borrow_record
ADD COLUMN return_condition VARCHAR(20)
CHECK (return_condition IN ('okay', 'late', 'damaged'));


ALTER TABLE return_lost_requests
DROP CONSTRAINT return_lost_requests_borrow_id_fkey;

ALTER TABLE return_lost_requests
ADD CONSTRAINT return_lost_requests_borrow_id_fkey
FOREIGN KEY (borrow_id) REFERENCES borrow_record(borrow_id)
ON DELETE set null;

ALTER TABLE Borrow_record
DROP CONSTRAINT borrow_record_copy_id_fkey;

ALTER TABLE Borrow_record
ADD CONSTRAINT borrow_record_copy_id_fkey
FOREIGN KEY (Copy_id) REFERENCES Book_Copy(Copy_id) ON DELETE SET NULL;

ALTER TABLE Book_Copy
DROP CONSTRAINT check_condition;

-- 2. Add a new check constraint with 'lost' included
ALTER TABLE Book_Copy
ADD CONSTRAINT check_condition
CHECK (Condition IN ('pages missing', 'fair', 'worn', 'lost'));

ALTER TABLE Fine_Calculated
ADD COLUMN pay_status VARCHAR(10) NOT NULL DEFAULT 'unpaid';

ALTER TABLE Notification
ADD COLUMN read_status BOOLEAN DEFAULT FALSE;

ALTER TABLE return_lost_requests
DROP CONSTRAINT return_lost_requests_request_type_check;

-- Step 2: Add new CHECK constraint with 'damaged'
ALTER TABLE return_lost_requests
ADD CONSTRAINT return_lost_requests_request_type_check
